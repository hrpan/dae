...
class CustomActorCriticPolicy(ActorCriticPolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if not self.shared_features_extractor:
            self.features_extractor_vf = self.features_extractor_class(self.observation_space)
            self.value_net = nn.Linear(self.features_extractor_vf.features_dim, 1)
            self.advantage_net = nn.Linear(self.features_extractor_vf.features_dim, self.action_space.n)
        else:
            self.value_net = nn.Linear(self.features_extractor.features_dim, 1)
            self.advantage_net = nn.Linear(self.features_extractor.features_dim, self.action_space.n)
        ...
        if (self.lr_vf is not None) and not self.shared_features_extractor:
            ...
            self.optimizer = self.optimizer_class(
                self.modules_pi.parameters(),
                lr=lr_schedule(1),
                **self.optimizer_kwargs
            )
            self.modules_vf = nn.ModuleList([self.features_extractor_vf, self.value_net, self.advantage_net])
            self.optimizer_vf = self.optimizer_class(
                self.modules_vf.parameters(),
                lr=self.lr_vf,
                **self.optimizer_kwargs
            )
        else:
    ...
    def extract_features_vf(self, obs: th.Tensor) -> th.Tensor:
        pobs = preprocess_obs(obs, self.observation_space, normalize_images=self.normalize_images)

        return self.features_extractor(pobs) if self.shared_features_extractor else self.features_extractor_vf(pobs)
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        latent = self.extract_features(obs)
        latent_vf = latent if self.shared_features_extractor else self.extract_features_vf(obs)
        mean_actions = self.action_net(latent)
        distribution = self.action_dist.proba_distribution(mean_actions)
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor, policies: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
        ...
        advantages = advantages_raw.gather(dim=1, index=actions.unsqueeze(1)) - th.sum(policies * advantages_raw, dim=1, keepdim=True)
        values = self.value_net(latent_vf)
        return values, advantages, log_probs, distribution.entropy()

    def evaluate_state(self, obs: th.Tensor, policies: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
        ...
        advantages_raw = self.advantage_net(latent_vf)
        advantages = advantages_raw - th.sum(policies * advantages_raw, dim=1, keepdim=True)
        values = self.value_net(latent_vf)
    ...
    def predict_value(self, obs: th.Tensor, policies: Optional[th.Tensor] = None) -> Tuple[th.Tensor, th.Tensor]:
